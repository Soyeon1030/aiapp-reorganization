...
요약(운영 현황) 대시보드는 자동 생성이며 **관리·설정 탭의 첫 화면**에 둔다 (2026-07-23 이동 — DB 탭은 저장소 열람 전용이므로): 방문자·회원(거래처)·주 데이터 건수 스탯 + 처리 대기 바로가기 + 최근 7일 추이 차트(방문자·주 데이터).
