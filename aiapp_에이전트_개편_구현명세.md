...
**도움말 장치 (2026-07-23 확정)**: DB 개념이 없는 사용자를 위한 안내는 전부 **플랫폼 고정 카피**로 해결한다 — 앱마다 AI가 판단·작성할 문장을 늘리지 않는다(생성 하네스 불변). 구성: ① 첫 진입 배너 "여기는 자동으로 쓰이는 장부예요"(계정당 1회, 닫기 가능) ② 탭 우측 ? 버튼으로 같은 배너 상시 재열람 ③ 처리·관리로의 이동 — 배너의 [관리 화면 가기]와 각 뷰 안내 문장의 "관리 화면" 링크가 관리·설정 운영 현황으로 딥링크(`settings.html?view=dash`, 설정 화면은 `?view=` 파라미터로 특정 뷰 직행을 지원). 표별 용어 설명(필드명 툴팁 등)은 앱별 생성 문장이 필요하므로 초기 범위에서 제외.
