...
**초기 범위 제외 — '내 프로젝트' 참조 첨부 (2026-07-21)**: 기존 프로젝트를 소스로 지정해 로고·숏폼 등 파생 작업에 참조시키는 기능은 구현 난이도로 1차 범위에서 제외한다. 미제공 기능은 화면에 두지 않는 원칙에 따라 B1 컴포저에서 칩도 제거했다. 파생 작업은 해당 프로젝트의 스튜디오 채팅 또는 브랜딩·홍보 탭에서 진행한다. 화면 레퍼런스: `design/b1.html`.
