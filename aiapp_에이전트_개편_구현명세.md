...
**표시 방식 전환 (2026-07-23 확정)**: 유형별 카드 UI는 폐기하고 모든 데이터 뷰를 **엑셀형 시트**(격자·행 번호·고정 헤더)로 통일한다 — 사용자에게 가장 익숙한 형태가 우선. 데이터 유형 메뉴는 사이드바가 아니라 **상단 탭**. 위 템플릿 계약(inbox/feed/directory/grid)은 화면 형태가 아니라 **시트의 컬럼 구성·기본 필터 축·행 처리 버튼(transitions)을 결정하는 용도**로 유지된다. 화면 레퍼런스: `design/db.html`.
