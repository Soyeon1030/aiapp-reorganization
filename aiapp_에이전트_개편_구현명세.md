...
**기능 관리 — 회원 관리 (2026-07-23 신설, 첫 사례)**: AI가 기획·생성 중에 붙인 기능마다 대응하는 관리 화면이 관리·설정의 "기능 관리" 그룹에 생긴다 — 회원 기능이 있으면 회원 관리가 생기는 식. 회원 관리 구성: ① **가입 방식 설정** — "바로 가입" vs "승인 후 가입" 세그먼트(승인제면 신규 가입이 승인 대기 상태로 들어오고 사이드바에 대기 배지) ② **회원 목록** — 검색(아이디·이름·연락처) + 컬럼: 아이디 · 이름 · 권한(관리자/일반) · 연락처 · 최근 접속 · 가입일 · 관리 ③ **메시지** — 체크박스로 회원 선택 → 알림톡 / SMS·MMS 채널 선택 후 발송(알림톡 실패 시 SMS 자동 대체) ④ **관리** — 대기 행은 [승인]/[거절], 일반 행은 [탈퇴](이중 확인: 한 번 더 눌러 확정), 관리자 본인은 탈퇴 불가. 화면 레퍼런스: `design/settings.html?view=members`.
