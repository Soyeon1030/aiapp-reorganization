...
**진행 표현 개정 (2026-07-24, Claude Agent SDK 정합)**: 고정 5단계 콘솔 카드(`console-card`)를 폐기하고 **할 일 위젯(`todo-card`)**으로 교체한다 — Agent SDK가 실제로 일하는 방식(요청마다 할 일 목록을 세우고, 하나씩 진행하며, 발견한 일을 목록에 추가) 그대로. 규칙: ① 목록은 에이전트가 세운다 — 개수·이름·순서를 화면이 가정하지 않고, 첫 서술에 "진행하면서 늘어날 수 있어요"를 명시 ② 항목 상태는 대기/진행 중(항상 1개, 스피너)/완료 3가지 — SDK todos의 pending/in_progress/completed와 1:1 ③ 중간 추가 항목은 하이라이트 + "추가됨" 태그 ④ **도구 실행은 접힌 작업 줄(`actline`)** — 같은 할 일 구간의 tool_use들을 묶어 쉬운 말 요약 한 줄, 펼치면 상세(파일 경로·코드 노출 금지). 묶음·번역은 SDK가 주지 않으므로 서버/클라이언트 책임 ⑤ thinking(추론) 블록은 비노출 — 서술 줄(text)이 추론의 대역 ⑥ 할 일 도구 계약은 SDK 내장 보장이 아니므로 커스텀 도구/시스템 프롬프트로 우리가 정의 ⑦ ✓ 체크 줄·되묻기(AskUserQuestion↔canUseTool)·일시정지·완료 보고 카드는 기존 유지 — 교체 대상은 진행 표현뿐. §2.2.3의 `console_update` 이벤트는 `todo_update`(전체 스냅샷)로 대체. 근거: Agent SDK agent-loop/streaming/user-input 문서 (2026-07-24 검증).
